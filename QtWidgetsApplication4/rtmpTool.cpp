...
int ff_rtmp_calc_digest_pos(const uint8_t* buf, int off, int mod_val,
    int add_val);
int rtmp_validate_digest(uint8_t* buf, int off)
{
    uint8_t digest[32];
    int ret, digest_pos;
    ...
    int* nb_prev_pkt, uint8_t hdr)
{
    ...
    if (ts_field == 0xFFFFFF) {
        int rsize=recv(socket, (char*)buf, 4, 0);
        if (rsize != 4)
            return AVERROR(EIO);
...
int ff_rtmp_packet_read_internal(int socket,RTMPPacket* p, int chunkSize,
    ...
    uint8_t hdr)
{
    ...
    while (1) {
        ...
        int ret = rtmp_packet_read_one_chunk(socket,p, chunkSize, prev_pkt,
            nb_prev_pkt, hdr);
...
int rtmp_handshake_imprint_with_digest(uint8_t* buf, int encrypted)
{
    int ret, digest_pos;
